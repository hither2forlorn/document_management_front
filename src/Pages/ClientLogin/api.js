...
function login({ username, password }) {
  ...
  server
    ...
    .catch((error) => {
      ...
      if (res?.status === 403 && res?.data?.mustChangePassword) {
        // ✅ Redirect to change-password page
        window.location.href = "/admin/users/change-password";
      } else {
        window.alert(res?.data?.message || "You cannot login to this system!");
      }
    });
}
