...
export default function HeaderPreview(props) {
  ...
  const fileAttachments = attachments.filter((row) => !row.fileType.includes("image", "parent"));

  return (
    <div className="justify-content-between d-flex align-items-center">
      {fileAttachments.length > 0 && (
        <Dropdown isOpen={state.isDropdownOpen} toggle={() => setState({ ...state, isDropdownOpen: !state.isDropdownOpen })}>
          <DropdownToggle caret>
            <i className="fas fa-file"></i> Select Files
          </DropdownToggle>
          <DropdownMenu
            style={{
              zIndex: 1020,
      ...
      <h5 className="m-0 pr-2">Preview File</h5>
