...
class AddDocument extends React.Component {
  ...
  handleSubmit = async (event) => {
    ...
    Object.keys(formData).forEach((key) => {
      if (!isNaN(key) && parseInt(key) !== formData.documentTypeId) {
        formData.document_index_values.push({
          documentIndexId: key,
          value: formData[key],
        });
      }
    ...
    addDocument(formData, (err, data) => {
      if (err || !data.success) {
        ...
        const hashId = A.getHash(data.id);
        let path = metaRoutes.documentsView + "?i=" + hashId;
  ...
  handleChange = (event) => {
    ...
    if (value == 1 || value == 2) {
      this.setState({ isRetailOrCorporate: true });
    } else {
      this.setState({ isRetailOrCorporate: false });
    }
  ...
  renderItems() {
    console.log(this.state.statusId, "this is status in add document");
    return (
        ...
        <Card className="shadow">
          ...
          <CardBody>
            ...
            <DocumentForm
              ...
              isRetailOrCorporate={this.state.isRetailOrCorporate}
